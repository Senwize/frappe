...
def get_pdf(html, options=None, output=None):
  ...
  try:
    ...
    runproc = subprocess.Popen(
        [f'{app_path}/html2pdf.js', '-', '-'], stdin=subprocess.PIPE, stdout=subprocess.PIPE)
    # Send HTML
    ...
    while runproc.returncode is None:
      if time.time() - timeout_start > pdf_generation_timeout:
...
def get_pdf_old(html, options=None, output=None):
	html = scrub_urls(html)
	html, options = prepare_options(html, options)
